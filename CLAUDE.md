...
10. **Version sync**: `package.json` and `electrobun.config.ts` versions must match the latest git tag. Run `bun run version:sync` after tagging a release, then commit the result.
