...
describe("parseSessionFile", () => {
  ...
  describe("minimal session", () => {
    ...
    it("captures assistant response preview", async () => {
      ...
      expect(result!.queries[0]!.assistantPreview).toBe(
    ...
    it("calculates cost based on model pricing", async () => {
      ...
      expect(result!.session.totalCost).toBeCloseTo(0.000_15, 6);
    });
  });
  ...
  describe("multi-query session", () => {
    ...
    it("calculates duration from timestamps", async () => {
      ...
      expect(result!.session.durationMs).toBe(121_000);
    });
    ...
    it("tracks context window usage per query", async () => {
      ...
      expect(usages[0]!.cumulativeTokens!).toBeLessThan(
        usages[1]!.cumulativeTokens!
      );
      expect(usages[1]!.cumulativeTokens!).toBeLessThan(
        usages[2]!.cumulativeTokens!
      );
    });
  });
  ...
  describe("session with tools", () => {
    ...
    it("extracts file operations from Read tools", async () => {
      ...
      const readOps = result!.fileOperations.filter(
        (f) => f.operation === "read"
      );
    ...
    it("extracts bash commands with category", async () => {
      ...
      const bunTest = result!.bashCommands.find(
        (b) => b.command === "bun test"
      );
  ...
  describe("session with caching", () => {
    it("tracks cache read and write tokens", async () => {
      ...
      expect(result!.session.totalCacheWrite).toBe(50_000);
      expect(result!.session.totalCacheRead).toBe(101_000); // 50000+51000
    });
  ...
  describe("session with errors", () => {
    it("captures API errors", async () => {
      ...
      const rateLimitError = result!.apiErrors.find(
    ...
    it("captures server errors", async () => {
      ...
      const serverError = result!.apiErrors.find(
        (e) => e.errorType === "server_error"
      );
